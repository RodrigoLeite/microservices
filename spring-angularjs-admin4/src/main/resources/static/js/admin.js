angular.module('admin', []).controller('home',
	...
	function($scope, $http) {
		...
		var computeDefaultTemplate = function(user) {
			$scope.template = user && user.roles && user.roles.indexOf("WRITER")>0 ? "write.html" : "read.html";		
		}
		...
		$scope.update = function() {
			$http.post('/resource/', {content: $scope.greeting.content})
				.success(function(data) {
					$scope.greeting = data;
				})
		}
